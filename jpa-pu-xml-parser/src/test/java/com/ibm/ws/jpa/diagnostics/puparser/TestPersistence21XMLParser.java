...
public class TestPersistence21XMLParser {
    ...
    @Test
    public void testJPA20() throws Exception {
        File testFile = new File(resDir, "persistence-2.1_01.xml");
        PersistenceDefinition pd = PersistenceUnitParser.parsePersistenceUnit(testFile);
        assertNotNull(pd);
        
        PUP_Persistence pxml = pd.getPersistence();
        assertNotNull(pxml);
        assertEquals("2.1", pxml.getVersion());
